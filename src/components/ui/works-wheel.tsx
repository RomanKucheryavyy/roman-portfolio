...
> {
  ...
  action?: string;
  /** Fires when the wheel settles on a new piece, so a host can narrate it. */
...
export function WorksWheel({
  ...
  // Latest-callback ref: the host may pass an inline arrow, and depending on it
  // would restart the animation loop on every render of the parent.
                ...
                >
                  <span className="bg-muted shadow-foreground/12 relative block size-full overflow-hidden rounded-lg shadow-[0_18px_40px_-18px_var(--tw-shadow-color)]">
                    {/* eslint-disable-next-line @next/next/no-img-element -- the
                        card is sized from a measured stage and transformed every
                        frame; next/image's fill wrapper fights both. */}
                    <img
                      src={item.image}
                      alt={item.title}
                      draggable={false}
